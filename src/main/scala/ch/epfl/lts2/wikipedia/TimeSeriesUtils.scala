...
object TimeSeriesUtils {
  /**
    ...
    */
  def smoothedZScore(y: Seq[Double], lag: Int, threshold: Double, influence: Double): Array[Int] = {

    // the results (peaks, 1 or -1) of our algorithm
    val signals = DenseVector.zeros[Int](y.length)

    // filter out the signals (peaks) from our original list (using influence arg)
    val filteredY:DenseVector[Double] = DenseVector(y.toArray)
    ...
    val v = DenseVector(y.take(lag).toArray)
